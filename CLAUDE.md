...
## Project Overview
Ledgerly is a minimalist, AI-powered expense tracker for Android that automatically extracts transaction data from SMS messages using on-device processing.
...
- **Edge-to-Edge**: All screens use LedgerlyScaffold with default TopAppBar for consistent system bar handling
- **Consistent UI**: LedgerlyScaffold provides default TopAppBar with options for title, navigation, actions, and transparency
...
1. **Location**: Add to `parser-core/src/main/kotlin/com/ledgerly/parser/core/bank/`
...
7. **Imports for parser-core**:
   - `com.ledgerly.parser.core.TransactionType`
   - `com.ledgerly.parser.core.ParsedTransaction`
   - `java.math.BigDecimal` for amounts

### Integration in main app:
- Use `com.ledgerly.tracker.data.mapper.toEntity()` to convert ParsedTransaction to TransactionEntity
