# Ledgerly Web - SMS Parser Tool

A web-based tool for testing and reporting SMS parsing issues for the Ledgerly app.
...
1. **Clone the repository**
   ...
   cd ledgerly-tracker/ledgerly-web/server
   ```
...
docker build -t ledgerly-web .

# Run the container
docker run -p 8080:8080 -e DATABASE_PASSWORD=your_password ledgerly-web
```

### Using systemd (Linux)

1. Create a service file at `/etc/systemd/system/ledgerly-web.service`:
   ```ini
   [Unit]
   Description=Ledgerly Web SMS Parser
   After=network.target

   [Service]
   Type=simple
   User=www-data
   WorkingDirectory=/opt/ledgerly-web
   Environment="DATABASE_PASSWORD=your_password"
   ExecStart=/usr/bin/java -jar /opt/ledgerly-web/server-all.jar
   Restart=always
...
2. Start the service:
   ```bash
   sudo systemctl enable ledgerly-web
   sudo systemctl start ledgerly-web
   ```
