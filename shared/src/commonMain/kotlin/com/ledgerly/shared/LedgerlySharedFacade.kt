...
import com.ledgerly.shared.data.SharedDataGraph
...
import com.ledgerly.shared.data.statement.SharedStatementImportResult
import com.ledgerly.shared.data.util.currentTimeMillis
import com.ledgerly.shared.data.util.monthStartEpochMillis
import com.ledgerly.shared.data.util.monthStartEpochMillisIST
import com.ledgerly.shared.domain.usecase.CreateManualTransactionUseCase
import com.ledgerly.shared.domain.usecase.ImportStatementUseCase
import com.ledgerly.shared.domain.usecase.ManualTransactionInput
import com.ledgerly.shared.domain.usecase.TransactionMutationResult
import com.ledgerly.shared.domain.usecase.UpdateManualTransactionUseCase
...
class LedgerlySharedFacade {
    companion object {
        val shared: LedgerlySharedFacade by lazy { LedgerlySharedFacade() }
    }
    ...
    fun addExpenseAndLoadHome(
        ...
        addTransactionAndLoadHome(
            ...
            occurredAtEpochMillis = com.ledgerly.shared.data.util.currentTimeMillis(),
            bankName = null,
            accountLast4 = null
        )
    }
    ...
    fun importStatementTextAndLoadHome(statementText: String): SharedHomeSnapshot = safeSnapshot {
        runBlocking {
            ...
            val debugParser = com.ledgerly.shared.data.statement.SharedStatementParserFactory.getParser(statementText)
            if (debugParser != null) {
                val debugParsed = debugParser.parse(statementText)
                debugParsed.take(3).forEachIndexed { i, tx ->
                    println("[Ledgerly DEBUG] Tx #${i + 1}: merchant=${tx.merchant}, amount=${tx.amountMinor}, timestamp=${tx.timestampEpochMillis}, type=${tx.transactionType}, bank=${tx.bankName}")
                }
            }
        ...
        return try {
            runBlocking {
                graph.transactionRepository.softDelete(
                    transactionId,
                    com.ledgerly.shared.data.util.currentTimeMillis()
                )
                true
            }
        } catch (_: Throwable) {
        ...
        return try {
            runBlocking {
                ...
                graph.categoryRepository.insertCategory(
                    com.ledgerly.shared.data.model.SharedCategory(
                        name = name,
                        colorHex = colorHex,
                        isSystem = false,
                        isIncome = isIncome,
        ...
        return try {
            runBlocking {
                ...
                graph.accountRepository.insertBalance(
                    com.ledgerly.shared.data.local.entity.SharedAccountBalanceEntity(
        ...
        return try {
            runBlocking {
                ...
                graph.accountRepository.upsertCard(
                    com.ledgerly.shared.data.local.entity.SharedCardEntity(
    ...
    fun addSubscriptionAndLoadHome(
        ...
        runBlocking {
            val now = com.ledgerly.shared.data.util.currentTimeMillis()
        ...
        return try {
            runBlocking {
                ...
                graph.ruleRepository.upsertRule(
                    com.ledgerly.shared.data.local.entity.SharedRuleEntity(
                        id = id,
                        name = name.trim(),
    ...
    private fun com.ledgerly.shared.data.local.entity.SharedRuleEntity.toRuleItem() =
        SharedRuleItem(
            id = id,
            name = name,
            conditionsJson = conditionsJson,
    ...
    private fun parseType(value: String): com.ledgerly.shared.data.model.SharedTransactionType {
        return com.ledgerly.shared.data.model.SharedTransactionType.entries.firstOrNull {
            it.name.equals(value, ignoreCase = true)
        } ?: com.ledgerly.shared.data.model.SharedTransactionType.EXPENSE
    }
    ...
    private fun com.ledgerly.shared.data.model.SharedTransaction.toItem() =
    ...
    private fun com.ledgerly.shared.data.local.entity.SharedAccountBalanceEntity.toAccountItem() =
        SharedAccountItem(
            bankName = bankName,
            accountLast4 = accountLast4,
            balanceMinor = balanceMinor,
    ...
    private fun com.ledgerly.shared.data.local.entity.SharedCardEntity.toCardItem() =
        SharedCardItem(
            id = id,
            cardLast4 = cardLast4,
            cardType = cardType,
    ...
    private fun com.ledgerly.shared.data.model.SharedCategory.toCategoryItem() =
        SharedCategoryItem(
            id = id,
            name = name,
            colorHex = colorHex,
