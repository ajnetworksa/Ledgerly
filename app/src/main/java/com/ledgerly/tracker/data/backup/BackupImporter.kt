package com.ledgerly.tracker.data.backup
...
import com.ledgerly.tracker.data.database.entity.*
...
class BackupImporter @Inject constructor(
    ...
    private val database: LedgerlyDatabase,
     ...
     */
    private suspend fun readBackupFile(uri: Uri): LedgerlyBackup {
        return withContext(Dispatchers.IO) {
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                val reader = BufferedReader(InputStreamReader(inputStream))
                val content = reader.readText()
                gson.fromJson(content, LedgerlyBackup::class.java)
     ...
     */
    private fun isCompatibleVersion(backup: LedgerlyBackup): Boolean {
        // For now, accept all v1.x backups
        return backup.format.startsWith("Ledgerly Backup v1")
    }
    
    /**
     * Replace all existing data with backup data
     */
    private suspend fun replaceAllData(backup: LedgerlyBackup): ImportResult {
     ...
     */
    private suspend fun mergeData(backup: LedgerlyBackup): ImportResult {
        var importedTransactions = 0
        var importedCategories = 0
