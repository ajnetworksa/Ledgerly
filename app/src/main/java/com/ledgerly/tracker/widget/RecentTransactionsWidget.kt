    ...
    private fun TransactionRow(item: RecentTransactionItem) {
        Row(
            ...
            val amountColor = when (item.transactionType) {
                TransactionType.INCOME -> LedgerlyWidgetTheme.transactionAmountColor(TransactionType.INCOME)
                TransactionType.TRANSFER -> GlanceTheme.colors.onSurfaceVariant
                else -> LedgerlyWidgetTheme.transactionAmountColor(item.transactionType)
            }
