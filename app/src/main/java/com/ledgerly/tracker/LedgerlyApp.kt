...
import com.ledgerly.tracker.navigation.AppLock
import com.ledgerly.tracker.navigation.Home
import com.ledgerly.tracker.navigation.OnBoarding
import com.ledgerly.tracker.navigation.LedgerlyNavHost
import com.ledgerly.tracker.ui.theme.LedgerlyTheme
import com.ledgerly.tracker.ui.viewmodel.AppLockViewModel
import com.ledgerly.tracker.ui.viewmodel.ThemeViewModel
import com.ledgerly.tracker.widget.RecentTransactionsWidgetUpdateWorker
...
fun LedgerlyApp(
    ...
    LaunchedEffect(appLockUiState.isLocked, appLockUiState.isLockEnabled) {
        if (appLockUiState.isLocked && appLockUiState.isLockEnabled) {
            ...
            if (currentRoute != AppLock::class.qualifiedName &&
                currentRoute != com.ledgerly.tracker.navigation.Settings::class.qualifiedName) {
    ...
    LaunchedEffect(editTransactionId) {
        editTransactionId?.let { transactionId ->
            navController.navigate(com.ledgerly.tracker.navigation.TransactionDetail(transactionId)) { launchSingleTop = true }
        }
    }
    ...
    LaunchedEffect(openAddTransaction) {
        if (openAddTransaction) {
            navController.navigate(com.ledgerly.tracker.navigation.AddTransaction) {
                launchSingleTop = true
            }
